...
sequence. If neither a fizz nor a buzz occurs, the sequence returns a number.
...
single sequence. As a demonstration, figure~\ref{fig:fizzbuzz_control} and
\ref{fig:fizzbuzz_result} shows a screenshot of a simple application, which
shows two rules and the resulting sequence. The implementation enables adding
new rules at runtime. 
...
and more exactly.
Listing~\ref{lst:diff_seq_usage} uses these functions to determine the the
...
\begin{itemize}
  \item |within| calculates recursively the slope of the |parabola| using the
    sequence |diffs| until a satisfying accuracy. Because of the lazyness of
