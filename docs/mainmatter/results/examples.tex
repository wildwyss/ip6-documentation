...
This section describes the implementation of the alpha-beta heuristic, an
...
possible. \\ 
In his paper, Hughes explains how laziness and higher order functions can be
used to implement this algorithm~\cite[p. 16]{hughes_why_1989}.
...
\label{sec:How does the Algorithm work?}
This section briefly introduces the topic and explains why laziness and
higher-order functions are crucial for this algorithm.
For a deeper understanding of the algorithm, you can either consult the paper
mentioned before or the section "Incremental Development" in Frege
Goodness~\cite{frege_goodness}.

\paragraph{Building a Game Tree} The idea of the algorithm is to build a tree
of possible playing fields. The start node in this tree is the current state of
the playing field. Its children are all possible playing fields, which can
arise when the next player has made his move. This results in a potentially
endless tree. All the next possible moves follow each node. Since this tree can
be huge, depending on the game, even infinitely large, this is only possible to
implement with laziness! 

The algorithm must estimate whether a playfield is good or bad for a player.
The simplest case for this is to judge based on the playing field whether a
player has already won at this level - in the case of tic tac toe, therefore,
when three of his symbols are next to each other. \\ 
A function (call it |evalFunction|) does this evaluation - if the algorithm has
won the game on a field, it evaluates to 1 if it has lost to -1, in all other
cases, to 0. \\ 
The |evalFunction| is mapped via the tree (this is where higher-order functions
come into play): Each node is assigned its static value. This results in a new
tree with the value of |evalFunction| assigned instead of all possible fields.

\paragraph{Pruning the huge Tree} The tree of possible moves and possible
...
the root which moves are good and which are not. The previously mentioned eval
function will evaluate to 0 for most nodes directly after the root. Therefore,
the computer also calculates all subsequent moves. The values of the lowest
calculated nodes are then propagated upwards to the root. 

The maximize function goes through to the lowest precalculated level. It
evaluates the board: when it is the computer's turn, it selects the board with
...
parent node as a result - and maximizes all nodes on this level according to
the same scheme. Ultimately, the root has the value with the best possible
chances of winning!

% Wie wird herausgefunden, ob ein Spielfeld gut ist? Der Algorithmus baut den
% Tree so auf, dass er eine möglichst gute Entscheidung treffen kann. Dabei geht
% er davon aus, dass sein Gegenspieler jedes Mal ebenfalls die beste Entscheidung
% trifft. Das funktioniert relativ einfach: Aus dem Spielfeld-Tree baut der
% Algorithmus einen neuen Tree indem jedem Spielfeld eine Zahl zugeordnet wird.
% Dieses aufbauen geschieht dadurch, dass eine statische Evaluationsfunktion über
% den Tree gemappt wird - hier kommt also die higher-order function ins Spiel.
% Die Evaluations sagt basierend auf dem Spielfeld, wie gut ein Spielzug ist. Bei
% einem unendlich grossen Tree muss das natülrich wieder lazy passieren. Der
% Algorithmus möchte den Tree aber so bauen, dass er für sich den besten Zug
% wählen kann. Das heisst, 
% subsubsection How does the Algorithm work? (end)
...
/**
 * This tree manages the possible game states.
 * @template _T_
