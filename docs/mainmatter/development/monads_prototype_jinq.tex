...
\subsubsection{Creating a Prototype for the Context} % (fold)
\label{sec:Creating a Prototype for the Context}
% subsubsection Creating a Prototype for the Context (end)
...
Listing~\ref{lst:jsdoc_maybe} adds the |MonadType| JSDoc to Maybe:

\begin{lstlisting}[
  style=ES6,
  caption=Adding JSDoc to Maybe,
  label={lst:jsdoc_maybe}
]

/** *'\label{line:jsdoc_maybe1}'*
 * @template _T_
 * @typedef { 
 *              ((f:<omitted>)  => (g:<omitted>) => _T_) *'\label{line:jsdoc_maybe2}'*
              & MaybeMonadType<_T_>    *'\label{line:jsdoc_maybe3}'*
            } MaybeType
 */

/** *'\label{line:jsod_maybe4}'*
...
What happens here? \\ 
Line~\ref{line:jsdoc_maybe1} and following specify the updated |MaybeType|.
|MaybeType| is a function, which receives to other functions as parameters
(Line~\ref{line:jsdoc_maybe2}). And it is also everything that is specified by
|MaybeMonadType| (Line~\ref{line:jsdoc_maybe3}))! \\
