...
list is empty. When returning |null|, the user has to explicitly deal with
...
lines~\ref{line:js_safeHead1}-\ref{line:js_safeHead2} evaluate the result of
...
argument. \\ 
This shows that |and| can emulate the behavior of |app|.
...
properties. Listing~\ref{lst:proto_maybe_js} now sets the prototype to the two
...
kinded types \cite{baeldung_higher-kinded_2020}, which would type definitions as defined by
...
carries defines these functions. |Object.setPrototypeOf| then adds it to the
sequence.
